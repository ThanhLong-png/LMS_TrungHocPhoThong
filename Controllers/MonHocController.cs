...
namespace LMS_THPT.Controllers
{
    public class MonHocController : Controller
    {
        ...
        public async Task<IActionResult> Details(int id)
        {
            ...
            // Lấy danh sách giáo viên có chuyên môn dạy môn học này
            var qualifiedGvIds = await _context.MonHocGiaoViens
                .Where(mg => mg.MonHocId == monHoc.Id)
                .Select(mg => mg.NguoiDungId)
                .Distinct()
                .ToListAsync();

            var gvList = await _context.Users
                .Where(u => u.ChucVu == "Giáo viên" && qualifiedGvIds.Contains(u.Id))
                .ToListAsync();
        ...
        public async Task<IActionResult> AddGiaoVien(int monHocId, string nguoiDungId, int[]? lopIds)
        {
            ...
            foreach (var lopId in lopIds)
            {
                ...
                if (lopThuc.HasValue)
                {
                    var lm = await _context.LopMonHocs.FirstOrDefaultAsync(x => x.LopId == lopThuc.Value && x.MonHocId == monHocId);
                    if (lm != null)
                    {
                        lm.GiaoVienId = nguoiDungId;
                    }
                    else
                    {
                        _context.LopMonHocs.Add(new LopMonHoc
                        {
                            LopId = lopThuc.Value,
                            MonHocId = monHocId,
                            GiaoVienId = nguoiDungId
                        });
                    }
                }
        ...
        public async Task<IActionResult> RemoveGiaoVien(int monHocId, string nguoiDungId, int? lopId)
        {
            ...
            foreach (var a in assignments)
            {
                ...
                // Cập nhật LopMonHoc thành null
                if (a.LopId.HasValue)
                {
                    var lm = await _context.LopMonHocs.FirstOrDefaultAsync(x => x.LopId == a.LopId.Value && x.MonHocId == monHocId);
                    if (lm != null)
                    {
                        lm.GiaoVienId = null;
                    }
        ...
        public async Task<IActionResult> Create(int KhoiId, string TenMonHoc)
        {
            ...
            // Kiểm tra môn đã tồn tại trong cùng khối chưa
            var tenChuanHoa = TenMonHoc.Trim().ToLower();
            var datonTai = await _context.DanhSachMonHoc
                .AnyAsync(m => m.KhoiId == KhoiId
                            && m.TenMonHoc.Trim().ToLower() == tenChuanHoa);

            if (datonTai)
            {
                TempData["Error"] = $"Môn học \"{TenMonHoc.Trim()}\" đã tồn tại trong khối này!";
                return RedirectToAction("Index");
            }
            ...
            TempData["Success"] = $"Thêm môn học \"{TenMonHoc.Trim()}\" thành công!";
            return RedirectToAction("Index");
        }
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            // Kiểm tra còn phân công giáo viên không
            if (monHoc.MonHocGiaoViens != null && monHoc.MonHocGiaoViens.Any())
            {
                TempData["Error"] = $"Không thể xóa \"{monHoc.TenMonHoc}\" vì còn giáo viên đang phụ trách!";
                return RedirectToAction("Index");
            }

            // Soft-delete
            monHoc.IsActive = false;
            await _context.SaveChangesAsync();

            TempData["Success"] = $"Đã xóa môn học \"{monHoc.TenMonHoc}\" thành công!";
            return RedirectToAction("Index");
        }
    }
}